...
def main(argv):
    ...
    parser.add_argument("--number_of_actions",
                        ...
                        default=5,
                        nargs="?",
                        help="Number of actions ti input")
    ...
    user_params = ParameterVector("θ", 5)
    ...
    fm1 = ZZFeatureMap(feature_dimension=5, reps=2, entanglement='full')
    ...
    cb_qkt = QKTCallback()
    spsa_opt = SPSA(maxiter=10, callback=cb_qkt.callback, learning_rate=0.0001, perturbation=0.1)

    # Instantiate a quantum kernel trainer.
    qkt = QuantumKernelTrainer(
        quantum_kernel=quant_kernel, loss="svc_loss", optimizer=spsa_opt, initial_point=[np.pi / 5, np.pi / 5, np.pi / 5, np.pi / 5, np.pi / 5]
    )

    # Train the kernel using QKT directly
    qka_results = qkt.fit(X_actions_train, y_train)
    ...
    # Predict the labels
    labels_test = qsvc.predict(X_actions_test)
    ...
    K = optimized_kernel.evaluate(X_actions_train)  # kernel matrix evaluated on the training samples
    K.to
    #K = qsvc.quantum_kernel
